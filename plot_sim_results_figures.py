...
def create_experiment_plot(experiment_result: dict, all_experiments: dict, figsize=(8, 8), rotate_90_deg: bool = False):
    ...
    if prior_map_object is not None and "additional_positions" in map_data:
        ...
        if len(prior_obj_positions) > 0:
            if rotate_90_deg:
                prior_obj_positions[:, 0], prior_obj_positions[:, 1] = prior_obj_positions[:, 1], prior_obj_positions[:, 0]
            plt.scatter(prior_obj_positions[:,0], prior_obj_positions[:,1], color='blue', marker='X', s=100, label="Prior Map Object")
    ...
    if experiment_result["app"] == "random":
        ...
        app = "DynaMem"
    text = r"\textbf{" + app + "}: " + experiment_result["experiment"]["goal"]["label"] + f" {'found' if success else 'not found'}\n"
    # plt.text(
    #     0.5,
    #     0.98,
    #     text,
...
def main():
    ...
    exp = select_experiments(
        data, app_name=["dynamem", "perceivesemantix", "random"], name_filter=["knife-hidden"], exclusive_name=["explore"]
    )
    fig_output_dir = Path("/home/benni/datasets/sim_results_syn_new/plots-ready")
    fig_output_dir.mkdir(parents=True, exist_ok=True)

    for e in exp:
        fig = create_experiment_plot(e, data, figsize=(6, 4))
        if fig:
    ...
    exp = select_experiments(
        data, app_name=["dynamem", "perceivesemantix", "random"], name_filter=["0026_chair_moved"], exclusive_name=["explore"]
    )
    for e in exp:
        fig = create_experiment_plot(e, data, figsize=(6, 4), rotate_90_deg=True)
        if fig:
            fig.savefig(fig_output_dir / f"{e['name']}.pdf")
            plt.close(fig)
